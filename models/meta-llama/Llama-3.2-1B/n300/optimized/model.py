...
"""
Optimized Llama 3.2 1B implementation in ttnn for N300.

This version uses 1D tensor parallel across a 1x2 mesh when available:
- QKV, gate, and up projections are column-parallel (width sharded) across devices.
- Output and down projections are row-parallel (height sharded) across devices, followed by all-reduce.

On hosts where the fabric topology is disconnected, set `TTNN_ALLOW_SYSTEM_MESH_FALLBACK=1`
to allow opening a 1x1 mesh device and running the same model in single-device mode.
...
def mesh_shape_to_axis(mesh_shape: tuple[int, int]) -> int:
    ...
    if mesh_shape[1] == 1 and mesh_shape[0] > 1:
        return 0
    raise ValueError(f"Expected 1D mesh shape, got {mesh_shape}")
...
def validate_parallel_config(config: ModelConfig, tp_size: int) -> None:
    if tp_size < 1:
...
class TtnnLlamaForCausalLM(torch.nn.Module, GenerationMixin):
    ...
    def __init__(self, hf_model, tt_device, max_seq_len: int = 2048):
        ...
        validate_parallel_config(self.tt_config, tp_size)
