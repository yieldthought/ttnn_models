# MODEL_BRINGUP.md - Mistral-7B-Instruct-v0.3 (t3000 optimized)

## Overview
This is the optimized T3000 path for `mistralai/Mistral-7B-Instruct-v0.3`.

- Model code: `models/mistralai/Mistral-7B-Instruct-v0.3/t3000/optimized/model.py`
- Demo log: `models/mistralai/Mistral-7B-Instruct-v0.3/t3000/optimized/demo.log`
- Eval log: `models/mistralai/Mistral-7B-Instruct-v0.3/t3000/optimized/eval.log`
- Parallelism: 1D tensor parallel across the 8-chip T3000 mesh (2x4)
...
env HF_HOME=/proj_sw/user_dev/moconnor/hf-cache \
  ...
  PYTHONUNBUFFERED=1 \
  python -u demo.py models/mistralai/Mistral-7B-Instruct-v0.3/t3000/optimized/model.py --max_seq_len 32768
```
...
env HF_HOME=/proj_sw/user_dev/moconnor/hf-cache \
  ...
  TT_METAL_RUNTIME_ROOT=/proj_sw/user_dev/moconnor/tt-metal \
  PYTHONUNBUFFERED=1 \
  python -u eval.py models/mistralai/Mistral-7B-Instruct-v0.3/t3000/optimized/model.py \
...
## Deferred changes
1. Decode-only sharded matmul program-config tuning (dram-sharded decode MLP / o-proj) was deferred; the current traced + L1 path already exceeds the performance targets with simpler code.

## Environment note
1. On this host, `/home/moconnor/bin/tt-smi` has a stale Python shebang (`bad interpreter`). Use `python3 -m tt_smi` for `-r` / `-ls`.
