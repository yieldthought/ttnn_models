...
"""
Functional Qwen3.5-35B-A3B bringup on n150.
...
class FullAttentionTT:
    ...
    def _apply_partial_rope_decode(self, q: ttnn.Tensor, k: ttnn.Tensor, start_pos: int) -> tuple:
        if self.rotary_dim == self.head_dim:
        ...
        q_rot = ttnn.experimental.rotary_embedding(q_rot, self.cos_cache, self.sin_cache, start_pos)
        k_rot = ttnn.experimental.rotary_embedding(k_rot, self.cos_cache, self.sin_cache, start_pos)
        q = ttnn.concat([q_rot, q_pass], dim=-1)
        k = ttnn.concat([k_rot, k_pass], dim=-1)
        return q, k
    ...
    def __call__(
        ...
        if is_prefill:
            ...
            q, k = self._apply_partial_rope_decode(q, k, start_pos)
...
class LinearAttentionTT:
    ...
    def __call__(self, x: ttnn.Tensor, start_pos: int, seq_len: int, cur_pos_tensor: Optional[ttnn.Tensor] = None) -> ttnn.Tensor:
        del start_pos
        del cur_pos_tensor
...
class SparseMoEHost:
    ...
    def _router_on_device(self, hidden_states: ttnn.Tensor) -> tuple:
        router_logits = ttnn.linear(hidden_states, self.router_weight)
        routing_logits, selected_experts = ttnn.topk(router_logits, k=self.top_k, dim=3, sorted=True)
    ...
    def __call__(self, x: ttnn.Tensor, seq_len: int) -> ttnn.Tensor:
        ...
        routing_weights_tt, selected_experts_tt = self._router_on_device(hidden_states_tt)
        ...
        routing_weights = ttnn.to_torch(routing_weights_tt).reshape(seq_len, self.top_k).to(torch.bfloat16)
        selected_experts = ttnn.to_torch(selected_experts_tt).reshape(seq_len, self.top_k).to(torch.long)
...
class DecoderLayer:
    ...
    def __call__(
        ...
        attn_out = self.attn(h, start_pos, seq_len, cur_pos_tensor)
