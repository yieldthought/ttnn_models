...
"""
Functional Qwen3-30B-A3B bringup in TTNN.

This model uses dynamic MoE expert streaming:
- Expert weights are converted/tilized to host TTNN tensors once per expert.
- Selected experts are copied host->device just-in-time into reusable buffers.
- Batch=1 inference is supported for prefill + decode.

This keeps expert conversion out of the token loop while avoiding loading all
experts on device at once.
"""
...
class SparseMoE:
    """
    Sparse MoE block with dynamic expert host->device loading.

    Expert weight conversion/tilization happens once per expert on host.
    Runtime per call:
    1) route tokens on host,
    2) copy selected expert weights host->device,
    3) run expert on routed token subsets.
    """
    ...
    def __init__(self, config: ModelConfig, layer_idx: int, state_dict: dict, tt_device):
        ...
        self.host_expert_cache = {}
        self.loaded_expert = None

        self.gate_proj_buffer = ttnn.from_torch(
            torch.zeros((1, 1, self.hidden_size, self.moe_intermediate_size), dtype=torch.bfloat16),
            dtype=ttnn.bfloat16,
            layout=ttnn.TILE_LAYOUT,
            device=tt_device,
            memory_config=ttnn.DRAM_MEMORY_CONFIG,
        )
        self.up_proj_buffer = ttnn.from_torch(
            torch.zeros((1, 1, self.hidden_size, self.moe_intermediate_size), dtype=torch.bfloat16),
            dtype=ttnn.bfloat16,
            layout=ttnn.TILE_LAYOUT,
            device=tt_device,
        ...
        self.down_proj_buffer = ttnn.from_torch(
            torch.zeros((1, 1, self.moe_intermediate_size, self.hidden_size), dtype=torch.bfloat16),
            dtype=ttnn.bfloat16,
            layout=ttnn.TILE_LAYOUT,
            device=tt_device,
            memory_config=ttnn.DRAM_MEMORY_CONFIG,
    ...
    def _prepare_host_expert(self, expert_idx: int):
        if expert_idx in self.host_expert_cache:
            return self.host_expert_cache[expert_idx]
        ...
        host_gate = ttnn.from_torch(
            gate_weight.T.unsqueeze(0).unsqueeze(0).to(torch.bfloat16).contiguous(),
            dtype=ttnn.bfloat16,
            layout=ttnn.TILE_LAYOUT,
        )
        host_up = ttnn.from_torch(
            up_weight.T.unsqueeze(0).unsqueeze(0).to(torch.bfloat16).contiguous(),
            dtype=ttnn.bfloat16,
            layout=ttnn.TILE_LAYOUT,
        )
        host_down = ttnn.from_torch(
            down_weight.T.unsqueeze(0).unsqueeze(0).to(torch.bfloat16).contiguous(),
            dtype=ttnn.bfloat16,
            layout=ttnn.TILE_LAYOUT,
        )
        self.host_expert_cache[expert_idx] = (host_gate, host_up, host_down)
        return self.host_expert_cache[expert_idx]

    def _load_expert_to_device(self, expert_idx: int) -> None:
        if self.loaded_expert == expert_idx:
            return
        host_gate, host_up, host_down = self._prepare_host_expert(expert_idx)
        ttnn.copy_host_to_device_tensor(host_gate, self.gate_proj_buffer)
        ttnn.copy_host_to_device_tensor(host_up, self.up_proj_buffer)
        ttnn.copy_host_to_device_tensor(host_down, self.down_proj_buffer)
        self.loaded_expert = expert_idx

    def _run_loaded_expert(self, x: ttnn.Tensor) -> ttnn.Tensor:
        gate = ttnn.silu(ttnn.linear(x, self.gate_proj_buffer))
        up = ttnn.linear(x, self.up_proj_buffer)
        product = ttnn.mul(gate, up)
        ttnn.deallocate(gate)
        ttnn.deallocate(up)
        out = ttnn.linear(product, self.down_proj_buffer)
    ...
    def __call__(self, x: ttnn.Tensor, seq_len: int) -> ttnn.Tensor:
        ...
        padded_seq = x.shape[2]
        hidden_states = ttnn.to_torch(x).reshape(padded_seq, self.hidden_size)[:seq_len].to(torch.bfloat16)

        router_logits = F.linear(hidden_states.to(torch.float32), self.gate_weight_torch)
        ...
        final_hidden_states = torch.zeros((seq_len, self.hidden_size), dtype=torch.bfloat16)
        ...
        for expert_idx_tensor in expert_hitted:
            ...
            self._load_expert_to_device(expert_idx)
            ...
            chunk_start = 0
            while chunk_start < token_idx.numel():
                chunk_end = min(chunk_start + TILE_SIZE, token_idx.numel())
                chunk_size = chunk_end - chunk_start

                chunk_state = torch.zeros((TILE_SIZE, self.hidden_size), dtype=torch.bfloat16)
                chunk_state[:chunk_size] = current_state[chunk_start:chunk_end]

                chunk_state_tt = ttnn.from_torch(
                    chunk_state.reshape(1, 1, TILE_SIZE, self.hidden_size),
                    dtype=ttnn.bfloat16,
                    layout=ttnn.TILE_LAYOUT,
                    device=self.tt_device,
                    memory_config=ttnn.DRAM_MEMORY_CONFIG,
                )
                chunk_hidden_tt = self._run_loaded_expert(chunk_state_tt)
                chunk_hidden = ttnn.to_torch(chunk_hidden_tt).reshape(TILE_SIZE, self.hidden_size)[:chunk_size]

                chunk_weight = current_weight[chunk_start:chunk_end]
                weighted_hidden = (chunk_hidden.to(torch.bfloat16) * chunk_weight).to(torch.bfloat16)
                final_hidden_states.index_add_(0, token_idx[chunk_start:chunk_end], weighted_hidden)

                ttnn.deallocate(chunk_state_tt)
                ttnn.deallocate(chunk_hidden_tt)

                chunk_start = chunk_end

        if seq_len < padded_seq:
            padded_hidden_states = torch.zeros((padded_seq, self.hidden_size), dtype=torch.bfloat16)
            padded_hidden_states[:seq_len] = final_hidden_states
        else:
            padded_hidden_states = final_hidden_states
