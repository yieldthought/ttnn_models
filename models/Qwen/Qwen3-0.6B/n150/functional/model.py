...
"""
Simple Qwen3 0.6B implementation in ttnn - 100% device execution.

This demonstrates a minimal bringup for Qwen3 with all compute on device.
...
- ttnn.transformer.scaled_dot_product_attention, ttnn.transformer.paged_scaled_dot_product_attention_decode
...
class Attention:
    ...
    def __init__(self, config: ModelConfig, layer_idx: int, state_dict: dict,
                 ...
                 tt_device, paged_attention_config: PagedAttentionConfig,
    ...
    def __call__(
        ...
        if is_prefill:
            ...
            # TT ops may pad head count to 32; trim to model width.
...
class TtnnQwen3ForCausalLM(torch.nn.Module, GenerationMixin):
    """
    Qwen3 model with 100% ttnn execution.
...
def build_model(hf_model, tt_device, max_seq_len: Optional[int] = None) -> TtnnQwen3ForCausalLM:
