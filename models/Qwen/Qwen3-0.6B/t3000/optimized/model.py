...
"""Qwen3 0.6B optimized TTNN model for T3000.
...
class Attention:
    ...
    def __init__(
        ...
        self.o_proj = self._load_weight(state_dict[f"{p}o_proj.weight"], parallel.shard_height_mapper, ATTN_WEIGHT_DTYPE)
    ...
    def _load_weight(self, w: torch.Tensor, mesh_mapper, dtype) -> ttnn.Tensor:
    ...
    def _load_qkv_weight(self, q_weight: torch.Tensor, k_weight: torch.Tensor, v_weight: torch.Tensor, mesh_mapper) -> ttnn.Tensor:
        ...
        return self._load_weight(qkv_weight, mesh_mapper, ATTN_WEIGHT_DTYPE)
...
class TtnnQwen3ForCausalLM(torch.nn.Module, GenerationMixin):
    """Qwen3 model with 100% ttnn execution and 1D tensor parallel on T3000."""
