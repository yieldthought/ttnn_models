...
"""
Simple Falcon3 7B Instruct implementation in ttnn - 100% device execution.
...
- ttnn.transformer.paged_scaled_dot_product_attention_decode: Decode attention
- ttnn.experimental.paged_fill_cache / ttnn.experimental.paged_update_cache: KV cache management
...
class TtnnFalcon3ForCausalLM(torch.nn.Module, GenerationMixin):
    """
    Falcon3 model with 100% ttnn execution.
...
def build_model(hf_model, tt_device, max_seq_len: Optional[int] = None) -> TtnnFalcon3ForCausalLM:
    """Build the ttnn model from a HuggingFace reference model."""
    return TtnnFalcon3ForCausalLM(hf_model, tt_device, max_seq_len)
