...
"""
Falcon3 7B Instruct implementation in ttnn for T3000.
...
class Attention:
    ...
    def __init__(
        ...
        self.n_kv_heads = self._pad_to_devices(self.original_num_kv_heads, parallel.num_devices)
    ...
    def _pad_to_devices(self, value: int, num_devices: int) -> int:
        if value % num_devices == 0:
            return value
        return ((value + num_devices - 1) // num_devices) * num_devices

    def _load_padded_weight(
        self,
        w: torch.Tensor,
        original_heads: int,
        padded_heads: int,
        ...
        w = w.reshape(padded_heads * self.head_dim, in_features)
        return self._load_weight(w, mesh_mapper)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if start_pos + seq_len > self.max_seq_len:
            raise ValueError(
                f"sequence length {start_pos + seq_len} exceeds cache length {self.max_seq_len}; "
