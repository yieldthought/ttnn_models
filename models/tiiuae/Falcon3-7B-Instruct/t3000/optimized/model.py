...
"""
Optimized Falcon3 7B Instruct implementation in ttnn for T3000.
...
This version uses 1D tensor parallel across a 2x4 mesh:
...
def pad_to_devices(value: int, num_devices: int) -> int:
    """Pad a count to divide evenly across devices."""
    if value % num_devices == 0:
        return value
    return ((value + num_devices - 1) // num_devices) * num_devices
...
class Attention:
    ...
    def __init__(
        ...
        self.original_num_heads = config.num_attention_heads
        self.original_num_kv_heads = config.num_key_value_heads
        if self.original_num_heads % self.original_num_kv_heads != 0:
            raise ValueError("num_attention_heads must be a multiple of num_key_value_heads")
        self.group_size = self.original_num_heads // self.original_num_kv_heads
        self.n_kv_heads = pad_to_devices(self.original_num_kv_heads, parallel.num_devices)
        self.n_heads = self.n_kv_heads * self.group_size
        ...
        self.padded_hidden_size = self.n_heads * self.head_dim
        ...
        q_weight = self._pad_weight_for_heads(
            state_dict[f"{p}q_proj.weight"],
            self.original_num_heads,
            self.n_heads,
        )
        k_weight = self._pad_weight_for_heads(
            state_dict[f"{p}k_proj.weight"],
            self.original_num_kv_heads,
            self.n_kv_heads,
        )
        v_weight = self._pad_weight_for_heads(
            state_dict[f"{p}v_proj.weight"],
            self.original_num_kv_heads,
            self.n_kv_heads,
        )
        self.qkv_proj = self._load_qkv_weight(q_weight, k_weight, v_weight, parallel.shard_width_mapper)
        self.o_proj = self._load_o_weight(state_dict[f"{p}o_proj.weight"], parallel.shard_height_mapper)
    ...
    def _pad_weight_for_heads(self, w: torch.Tensor, original_heads: int, padded_heads: int) -> torch.Tensor:
        if padded_heads == original_heads:
            return w
        in_features = w.shape[1]
        w = w.reshape(original_heads, self.head_dim, in_features)
        pad_heads = padded_heads - original_heads
        pad = torch.zeros((pad_heads, self.head_dim, in_features), dtype=w.dtype)
        w = torch.cat([w, pad], dim=0)
        return w.reshape(padded_heads * self.head_dim, in_features)

    def _load_o_weight(self, w: torch.Tensor, mesh_mapper) -> ttnn.Tensor:
        if self.padded_hidden_size == w.shape[1]:
            return self._load_weight(w, mesh_mapper)
        pad = torch.zeros((w.shape[0], self.padded_hidden_size - w.shape[1]), dtype=w.dtype)
...
class TtnnFalconForCausalLM(torch.nn.Module, GenerationMixin):
    """
    Falcon3 model with 100% ttnn execution and 1D tensor parallel on T3000.
    ...
    def __init__(self, hf_model, tt_device, max_seq_len: Optional[int] = None):
        ...
        if max_seq_len is None:
            ...
            if max_seq_len is None:
                raise ValueError("max_seq_len is required when HF config has no max_position_embeddings")
        ...
        original_num_heads = self.tt_config.num_attention_heads
        original_num_kv_heads = self.tt_config.num_key_value_heads
        if original_num_heads % original_num_kv_heads != 0:
            raise ValueError("num_attention_heads must be a multiple of num_key_value_heads")
        group_size = original_num_heads // original_num_kv_heads
        padded_kv_heads = pad_to_devices(original_num_kv_heads, self.parallel.num_devices)
        padded_heads = padded_kv_heads * group_size
        local_heads = padded_heads // self.parallel.num_devices
        local_kv_heads = padded_kv_heads // self.parallel.num_devices
        ...
        self.decode_q_rope_seq = local_heads * TILE_SIZE
        self.decode_k_rope_seq = local_kv_heads * TILE_SIZE
...
def build_model(hf_model, tt_device, max_seq_len: Optional[int] = None) -> TtnnFalconForCausalLM:
    """Build the ttnn model from a HuggingFace reference model."""
    return TtnnFalconForCausalLM(hf_model, tt_device, max_seq_len)
