...
"""
Minimal Arcee AFM-4.5B implementation in ttnn on N300.
...
This N300 version uses 1D tensor parallel across a 1x2 mesh:
- QKV and MLP up projections are column-parallel (width sharded).
- Output and MLP down projections are row-parallel (height sharded) with all-reduce.
"""
...
class TtnnArceeForCausalLM(torch.nn.Module, GenerationMixin):
    """Arcee model with 100% ttnn execution and 1D tensor parallel on N300."""
    ...
    def __init__(self, hf_model, tt_device, max_seq_len: Optional[int] = None):
        ...
        self.vocab_size_padded = math.ceil(self.vocab_size / self.parallel.num_devices) * self.parallel.num_devices
        if self.vocab_size_padded != self.vocab_size:
            pad_rows = self.vocab_size_padded - self.vocab_size
