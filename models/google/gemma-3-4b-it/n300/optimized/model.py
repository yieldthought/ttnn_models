...
"""
Optimized Gemma 3 4B Instruct implementation in ttnn with 1D tensor parallel on N300.
...
class TtnnGemma3ForCausalLM(torch.nn.Module, GenerationMixin):
    """
    Gemma 3 model with 100% ttnn execution and 1D tensor parallel on N300.
    ...
    def __init__(self, hf_model, tt_device, max_seq_len: Optional[int] = None):
        ...
        self.n_local_heads = self.tt_config.num_attention_heads // self.parallel.num_devices
        self.n_local_kv_heads = self.tt_config.num_key_value_heads // self.parallel.num_devices
        self.decode_q_rope_seq = TILE_SIZE * self.n_local_heads
        self.decode_k_rope_seq = TILE_SIZE * self.n_local_kv_heads
    ...
    def _update_decode_rope_buffers(self, start_pos: int) -> None:
        def update(cos_cache, sin_cache, cos_q, sin_q, cos_k, sin_k) -> None:
    ...
    def prefill_logits_last_device(self, input_ids: torch.Tensor, use_cache: bool = True) -> tuple[torch.Tensor, object]:
        ...
        logits = self._logits_to_torch(logits_device).reshape(batch, 1, -1)[:, 0, :]
        if self.tt_config.final_logit_softcapping is not None:
            softcap = self.tt_config.final_logit_softcapping
            logits = torch.tanh(logits / softcap) * softcap
        ttnn.deallocate(logits_device)
