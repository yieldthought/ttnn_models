...
"""
Optimized ALLaM 7B Instruct preview implementation in ttnn on T3000.
...
WEIGHT_DTYPE_NAME = os.environ.get("TTNN_WEIGHT_DTYPE", "bfloat8_b").lower()
if WEIGHT_DTYPE_NAME in ("bf16", "bfloat16"):
...
DECODE_MATMUL_FIDELITY = os.environ.get("TTNN_DECODE_MATMUL_FIDELITY", "").strip().lower()
DECODE_MATMUL_CORE_GRID = os.environ.get("TTNN_DECODE_MATMUL_CORE_GRID", "").strip().lower()


def _parse_core_grid(spec: str):
    if not spec:
        return None
    sep = "x" if "x" in spec else ","
    parts = [p.strip() for p in spec.split(sep)]
    if len(parts) != 2:
        raise ValueError(f"TTNN_DECODE_MATMUL_CORE_GRID must be 'YxX' or 'Y,X', got '{spec}'")
    return ttnn.CoreGrid(y=int(parts[0]), x=int(parts[1]))


def _decode_matmul_kernel_config():
    if DECODE_MATMUL_FIDELITY in ("", "default"):
        return None
    if DECODE_MATMUL_FIDELITY == "hifi2":
    ...
    if DECODE_MATMUL_FIDELITY == "lofi":
        return ttnn.WormholeComputeKernelConfig(
    ...
    raise ValueError(f"Unsupported TTNN_DECODE_MATMUL_FIDELITY='{DECODE_MATMUL_FIDELITY}'")


DECODE_MATMUL_KERNEL_CONFIG = _decode_matmul_kernel_config()
DECODE_MATMUL_CORE_GRID_CONFIG = _parse_core_grid(DECODE_MATMUL_CORE_GRID)


def _decode_linear(x: ttnn.Tensor, w: ttnn.Tensor, memory_config=None) -> ttnn.Tensor:
    kwargs = {}
    if memory_config is not None:
        kwargs["memory_config"] = memory_config
    if DECODE_MATMUL_CORE_GRID_CONFIG is not None:
        kwargs["core_grid"] = DECODE_MATMUL_CORE_GRID_CONFIG
    if DECODE_MATMUL_KERNEL_CONFIG is not None:
        kwargs["compute_kernel_config"] = DECODE_MATMUL_KERNEL_CONFIG
...
def mesh_shape_to_axis(mesh_shape: tuple[int, int]) -> Optional[int]:
    """Return mesh axis for 1D parallel, or None when reducing over 2D mesh."""
    if mesh_shape == (1, 1):
        return 0
...
class Attention:
    ...
    def __call__(
        ...
        if is_prefill:
            ...
            qkv = _decode_linear(x, self.qkv_proj, memory_config=DECODE_MEMORY_CONFIG)
        ...
        if is_prefill:
            ...
            out = _decode_linear(attn_out, self.o_proj)
...
class TtnnALLaMForCausalLM(torch.nn.Module, GenerationMixin):
    """ALLaM model with 100% ttnn execution and tensor parallel on T3000."""
