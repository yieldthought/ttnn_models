...
"""
Optimized ALLaM 7B Instruct preview implementation in ttnn on N150.
...
def _decode_linear(x: ttnn.Tensor, w: ttnn.Tensor, memory_config) -> ttnn.Tensor:
...
class Attention:
    ...
    def __call__(
        ...
        return _decode_linear(attn_out, self.o_proj, memory_config=DECODE_MEMORY_CONFIG)
...
class TtnnALLaMForCausalLM(torch.nn.Module, GenerationMixin):
    """ALLaM model with 100% ttnn execution and traced decode path on N150."""
    ...
    def __init__(self, hf_model, tt_device, max_seq_len: Optional[int] = None):
        ...
        if max_seq_len is None:
            ...
            if max_position_embeddings is not None and max_seq_len > max_position_embeddings:
                raise ValueError(f"max_seq_len {max_seq_len} exceeds max_position_embeddings {max_position_embeddings}")
